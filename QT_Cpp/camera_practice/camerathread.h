...
#include <QAbstractVideoBuffer> // 비디오 데이터를 추상화하여 데이터 버퍼 참조하는데 사용
#include <QVideoFrame>
#include <QImage>
#include <QString>
...
public:
    ...
    void request_ai(); //현재 카메라 프레임을 AI로 인식
    bool save_current_image(conts QString& path); //현재 이미지 파일 저장

signals:
	// 가공이 완료된 이미지를 메인 윈도우(GUI)로 보낼 때 발생하는 Qt 시그널
    void send_image(const QImage &image);

    void send_ai_result(const QString& result); //AI 결과 받는 시그널
...
private:
    ...
    cv::Mat make_square_digit(const cv::Mat& digit); //숫자 이미지를 정사각형으로 만듬(행렬로(OpenCV))
    cv::Mat qimage_to_mat(const QImage& image); //이미지를 숫자로 표현(행렬로(openCV))
    ...
    QImage m_current_image; //현재 이미지를 저장하여 이를 변환 AI로

    cv::dnn::Net m_model; //AI 모델(DNN을 표현하는 클래스)
